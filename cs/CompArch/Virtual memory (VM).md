...
- В процессе выполнения инструкции, которая требует доступа к памяти, CPU обращается к RAM, генерируя виртуальный адрес (VA), который затем преобразуется в соответсвующий физический адрес.
